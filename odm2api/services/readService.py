...
class ReadODM2(ServiceBase):
    ...
    #TODO - filering
    def read_queryset(self, obj, filters=None):
        """
        pass a class from ODM2Models and return the objects
        
        obj : python class from ODM2Models        
        """
        #Should probably return objects instead of collection results
        return self._session.query(obj.__table__)
    
    #TODO - filtering of some sort
    def read_dataframe(self, obj, filters=None):
        """
        pass a class from ODM2Models and return a pandas DataFrame
        ...
        dict_functions =
            {'People':self.__import_person__()}
        
        if filters is None:
            df = pd.read_sql_table(
                table_name = obj.__tablename__,
                schema = obj.__schema__,
                con=self._session.get_bind()
                )
        else:
            #sql = "SELECT * FROM obj.__tablename__" #We'd be writing custom SQL - might be issue depending on flavor of SQL
            df = pd.read_sql_table(
                table_name = obj.__tablename__,
                schema = obj.__schema__,
                con=self._session.get_bind()
...
#PRT - I'll review and see what we need to keep - comment out for now
...
#        self.Name = person.PersonFirltstName + ' ' + person.PersonLastName
